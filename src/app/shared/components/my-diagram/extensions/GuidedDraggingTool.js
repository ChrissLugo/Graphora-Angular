...
export class GuidedDraggingTool extends go.DraggingTool {
	...
	showHorizontalMatches(part, guideline, snap) {
		...
		if (bestObj !== null) {
			...
			const bestPoint = new go.Point().setRectSpot(
				bestBounds,
				bestOtherSpot
			);
			if (bestSpot === go.Spot.Center) {
				...
				if (guideline) {
					this.guidelineHcenter.position = new go.Point(
						x0,
						bestPoint.y
					);
				...
				if (guideline) {
					this.guidelineHbottom.position = new go.Point(
						x0,
						bestPoint.y
					);
	...
	showVerticalMatches(part, guideline, snap) {
		...
		if (bestObj !== null) {
			...
			if (bestSpot === go.Spot.Center) {
				...
				if (guideline) {
					this.guidelineVcenter.position = new go.Point(
						bestPoint.x,
						y0
					);
				...
				if (guideline) {
					this.guidelineVleft.position = new go.Point(
						bestPoint.x,
						y0
					);
				...
				if (guideline) {
					this.guidelineVright.position = new go.Point(
						bestPoint.x,
						y0
					);
