...
export class RotateMultipleTool extends go.RotatingTool {
	...
	doActivate() {
		...
		this._centerPoint = diagram.computePartsBounds(
			diagram.selection
		).center;
	...
	walkTree(part, infos) {
		...
		const dist = Math.sqrt(
			this._centerPoint.distanceSquaredPoint(part.location)
		);
	...
	rotate(newangle) {
		...
		const node =
			this.adornedObject !== null ? this.adornedObject.part : null;
		if (node === null) return;
		...
		this._initialInfo.each((kvp) => {
			...
			if (e.control || e.meta) {
				...
				const offsetX =
					partInfo.distance *
					Math.cos(radAngle + partInfo.placementAngle);
				const offsetY =
					partInfo.distance *
					Math.sin(radAngle + partInfo.placementAngle);
				// move part
	...
	computeRotate(newPoint) {
		...
		if (e.control || e.meta) {
			...
			angle =
				this._centerPoint.directionPoint(newPoint) - this._initialAngle;
		}
